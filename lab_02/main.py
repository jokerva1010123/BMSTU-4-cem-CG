...
def check(answer):
    ...
    for i in answer:
        if i not in correct:
            show_error("Возможно, вы ввели некорректные символы.")
            return False
    return True
...
def int_answer(answer):
    try:
        ...
        show_error("Возможно, у вас вещественное значение или пустой ввод.")
        return false
...
def float_answer(answer):
    try:
        ...
        show_error("Возможно, у вас пустой ввод.")
        return false
...
def move(dx, dy):
    ...
    global cancel_matrix
    cancel_matrix = cancel_func(matrix_mov)

    for i in range(len(list_point)):
        list_point[i] = mul_vector(list_point[i], matrix_mov)
    for i in range(len(list_circle)):
        list_circle[i] = mul_vector(list_circle[i], matrix_mov)
    show_scene()

def rotation():
    angle = entry_angle.get()
    if (check(angle) == False):
        return
    ...
    matrix_res = mul_matrix(matrix_mov, matrix)
    matrix_mov[2][0], matrix_mov[2][1] = -dx, -dy
    matrix_res = mul_matrix(matrix_res, matrix_mov)

    global cancel_matrix

    for i in range(len(list_point)):
        list_point[i] = mul_vector(list_point[i], matrix_res)
    for i in range(len(list_circle)):
        list_circle[i] = mul_vector(list_circle[i], matrix_res)
    cancel_matrix = cancel_func(matrix_res)
...
def scale():
    ...
    if kx == 0 or ky == 0:
        show_error("При таких значениях рисунка не будет!")
        return
...
def cancel():
    global cancel_matrix
    for i in range(len(list_point)):
        list_point[i] = np.dot(list_point[i], cancel_matrix)
    for i in range(len(list_circle)):
        list_circle[i] = np.dot(list_circle[i], cancel_matrix)
    cancel_matrix = [[1, 0, 0],
                      [0, 1, 0],
                      [0, 0, 1]]
    show_scene()
...
def show_scene():
    ...
    canv.create_line(max_size/2, max_size, max_size/2, 0, width=2, arrow=LAST)
    canv.create_line(0, max_size / 2, max_size,
                     max_size / 2, width=2, arrow=LAST)
